...
tx_austin['state'] = 'TX'
tx_austin['dist_type'] = 'School Districts'
...
def relative_distance_figure(all_data, census_col, dp_col, total_col, race_eth, minority_title):
    ...
    types = ['County BOS', 'City', 'County Comm', 'S.D.', 'School District', 'Comm Ct/Cons/JP', 'Comm Ct/JP','School Districts', 'Community College', 'Cons/JP', 'Groundwater','Hospital']
    fig, ax = plt.subplots()
    plt.rcParams.update({'font.size': 16})
    ax.yaxis.set_major_formatter(PercentFormatter(decimals = 1))
    plt.ylim([-10.25,10.25])
    for i in types:
...
#make table
maps = all_data.map.unique()
total_df = pd.DataFrame(columns=['jurisdiction','district type', 'num of districts', 'max abs difference', 'mean difference', 'mean abs difference','standard deviation'])
...
#save table
total_df.to_csv('/Users/hwheelen/Documents/GitHub/EIL-DP_Analysis/DOJ Jurisdictions/Output/DOJ_jurisdictions_memo_table.csv')
