...
- parses into separate data frames for all 50 states
- groups data frame by block geoid
- runs for all states and writes to file

- completed for all 50 states
...
def getStateData(st_fips, csize, fname):
    ...
    and search for records from the state
    
    return the full dataframe with all records from the state
...
def cleanData(df):
    ...
    take a dataframe with DP data from a single state
    clean the column names
    sort by GEOID to get 1 row for each populated block
    
    return data frame grouped by GEOID for a single state and ε level
...
sheet4_5 = 'D:/Census/ppmf_20210428_eps4-5_P.csv'
sheet12_2 = 'D:/Census/ppmf_20210428_eps12-2_P.csv'

chunksize = 10 ** 6


#### RUN DATA FOR ALL STATES
...
for i in range(0, len(abbrev_list)):
    ...
    st4_5_sort.to_csv('D:/Census/CensusDP_{0}_4_5.csv'.format(abbrev_list[i]))
    st12_2_sort.to_csv('D:/Census/CensusDP_{0}_12_2.csv'.format(abbrev_list[i]))
